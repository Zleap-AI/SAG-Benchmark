...
async def run_batch_search(
    ...
    effective_concurrency = max(1, min(max_concurrency, effective_bench_size))
    ...
    # 按 bench_size 分批创建任务，保持结果顺序，并在批次边界触发回调
    for batch_start in range(0, total, effective_bench_size):
        batch_end = min(batch_start + effective_bench_size, total)
        batch_items = [
            (i, questions[i])
        ...
        if effective_concurrency == 1:
            batch_results = []
            for i, q in batch_items:
                batch_results.append(await search_one(i, q))
        else:
            batch_results = await asyncio.gather(
                *(search_one(i, q) for i, q in batch_items)
            )
...
async def main():
    ...
    parser.add_argument("--strategy", type=str, required=True,
                        choices=["atomic", "multi", "multi_es", "multi1", "hopllm", "vector"],
                        help="搜索策略")
    ...
    parser.add_argument("--mode", type=str, choices=["fast", "precise"], default="fast",
                        help="multi_es mode: fast=spaCy+小规模扩展, precise=spaCy+LLM filter")
