...
# 修改内容：_create_openai_config 注入 enable_thinking:false（由 GRAPHRAG_DISABLE_THINKING=1 控制）。
...
def _create_openai_config(config: LanguageModelConfig, azure: bool) -> OpenAIConfig:
    ...
    # 复现 HippoRAG 时对齐其设置：qwen3.6-flash 默认开思考模式，
    # HippoRAG 每次调用都传 extra_body={"enable_thinking": False} 关掉它。
    # GraphRAG 走 fnllm，无配置字段可传 extra_body，这里按环境变量注入。
    # 底层 fnllm 用 client.chat.completions.create(**chat_parameters)，
    # openai SDK 原生识别 extra_body，键会被透传给服务端。
    if os.getenv("GRAPHRAG_DISABLE_THINKING") == "1":
        # vLLM 部署的 Qwen3 系列不认顶层 enable_thinking，必须放进
        # chat_template_kwargs 才会透传给 chat template 关掉思考；
        # 否则模型把 max_tokens 全花在隐藏推理上，content 返回 None，
        # 触发 BaseModelOutput 的 pydantic string_type 校验失败。
        chat_parameters["extra_body"] = {
