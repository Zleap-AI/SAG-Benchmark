...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    # namespace -> 中文语义（embedding 在做什么）
    _NS_SEMANTIC = {
        "entities": "实体向量化",
        "relationships": "超边/关系向量化",
        "chunks": "chunk 向量化",
    }

    def _ns_label(self) -> str:
        return self._NS_SEMANTIC.get(self.namespace, self.namespace)
    ...
    async def upsert(self, data: dict[str, dict]):
        logger.info(f"[embedding] {self._ns_label()}：对 {len(data)} 条 {self.namespace} 做向量化并写入向量库")
        if not len(data):
        ...
        logger.debug(f"[embedding] {self._ns_label()}：拆成 {len(batches)} 批（每批≤{self._max_batch_size}），并发调 embedding 服务")
        ...
        logger.debug(f"[embedding] {self._ns_label()}：{len(list_data)} 条向量写入完成")
        return results

    async def query(self, query: str, top_k=5):
        logger.debug(f"[embedding] query 向量化（{self._ns_label().replace('向量化','检索')}）：对查询文本做 embedding，top_k={top_k} | query[:60]={query[:60]!r}")
        ...
        logger.debug(f"[embedding] {self._ns_label().replace('向量化','检索')}：召回 {len(results)} 条")
        return results
