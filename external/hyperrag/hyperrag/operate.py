...
def _is_unrecoverable_content_error(e: BaseException) -> bool:
    """判断异常是否是「内容审核 / 不可重试」类错误，应直接跳过该 chunk。

    302ai 等网关对被审核拦截的请求返回 400 + data_inspection_failed / inappropriate content。
    这类错误对同一输入恒定触发，重试无意义；若不跳过会冒泡到 asyncio.gather 让整批
    insert 事务回滚中断。识别依据：解压后的响应文本里含审核关键字。
    """
    texts = []
    ...
    if resp is not None:
        ...
        if raw is not None:
            try:
                from .llm import _decompress_body

                body = _decompress_body(
                    raw, resp.headers.get("content-encoding") if hasattr(resp, "headers") else None
    ...
    keywords = (
        ...
        "content policy",
        "sensitive content",
        "output data may contain inappropriate",
    )
    return any(k in blob for k in keywords)


def _is_transient_exhausted_error(e: BaseException) -> bool:
    """判断是否「瞬时/可重试」类错误——这类错误对同一输入不恒定触发，重试有意义；
    但若能走到 _process_single_content 的 except，说明 tenacity 已重试耗尽（默认 5 次）
    仍失败，或错误本身不在 tenacity 重试列表里（如 524 Cloudflare 超时、5xx 服务端错误）
    直接冒泡。继续向上会让 asyncio.gather 整批 insert 事务回滚，前功尽弃。

    此时跳过该 chunk（原文仍进 chunks_vdb），保证整批能落盘。
    用类型名 + 解压后的响应文本匹配，避免硬依赖 openai 异常类型的 import。
    """
    name = type(e).__name__
    transient_types = (
        "Timeout", "APITimeoutError", "TimeoutError",
        "ConnectError", "APIConnectionError", "ConnectionError",
        "RateLimitError", "InternalServerError", "APIStatusError",
    )
    if any(t in name for t in transient_types):
        return True
    # 进一步看响应体：524(Cloudflare origin 超时) / 5xx / retryable 等
    resp = getattr(e, "response", None)
    ...
    if resp is not None:
        try:
            ...
            if raw is not None:
                ...
                if any(k in body for k in ("524", "retryable", "origin_response_timeout",
                                           "timeout occurred", "overloaded", "service unavailable")):
                    return True
        except Exception:
            pass
    return False
...
def _is_llm_api_error(e: BaseException) -> bool:
    """判断是否「来自 LLM 调用」的异常（API 返回的错误），区别于 hyperrag 自身的代码 bug。

    到了 _process_single_content 的 except、且重试已耗尽时，凡是 LLM/API 侧的错误
    （无论 400/500/超时/连接/限流，无论响应体能否解压）都应跳过该 chunk，绝不冒泡
    让整批 insert 回滚。只有真正的代码 bug（KeyError/AttributeError 等）才 raise。
    """
    name = type(e).__name__
    # openai SDK 的所有 API 异常基类名 / httpx HTTPStatusError
    api_markers = (
        ...
        "APITimeoutError", "Timeout", "HTTPStatusError", "Response",
    )
    if any(m in name for m in api_markers):
        return True
    # 5xx/4xx 有 response 对象的，也算 API 错误
    resp = getattr(e, "response", None)
    status = getattr(resp, "status_code", None) if resp is not None else None
...
def _readable_err_short(e: BaseException, n: int = 300) -> str:
    resp = getattr(e, "response", None)
    if resp is None:
    ...
    try:
        ...
        body = str(e)
    return f"{type(e).__name__} [HTTP {status}] {body}"[:n]
...
async def _handle_entity_summary(
    ...
    try:
        # 单个 summary 的 LLM 调用加超时，防止卡死整个合并阶段（这是 gather 后卡死的真凶）
        summary = await asyncio.wait_for(
            use_llm_func(use_prompt, max_tokens=summary_max_tokens),
            timeout=SUMMARY_TIMEOUT,
        )
    except asyncio.TimeoutError:
        logger.warning(f"[summary-timeout] {entity_or_relation_name} 摘要超时(>{SUMMARY_TIMEOUT}s)，用原始描述兜底")
        summary = use_description
    except Exception as e:
        logger.warning(f"[summary-error] {entity_or_relation_name} 摘要失败：{_readable_err_short(e)}")
        summary = use_description
    if summary is None:
...
async def _handle_entity_additional_properties(
    ...
    try:
        ...
        logger.warning(f"[summary-timeout/error] {entity_name} 附加属性摘要失败({type(e).__name__})，用原始兜底")
...
async def _handle_relation_summary(
    ...
    try:
        ...
        logger.warning(f"[summary-timeout] 关系 {relation_name} 摘要超时(>{SUMMARY_TIMEOUT}s)，用原始描述兜底")
        summary = use_description
    except Exception as e:
        logger.warning(f"[summary-error] 关系 {relation_name} 摘要失败：{_readable_err_short(e)}")
        summary = use_description
    if summary is None:
...
async def _handle_relation_keywords_summary(
    ...
    try:
        ...
        logger.warning(f"[summary-timeout/error] 关系 {relation_name} 关键词摘要失败({type(e).__name__})，用原始兜底")
...
# 单个 chunk 实体抽取的硬超时（秒）。超时则跳过该 chunk，根治：
# ① LLM 返回畸形结果导致解析死循环；② 单次 LLM 调用卡死无响应。
# 正常一个 chunk（主体抽取+glean+关系）约 1-4 分钟，给 10 分钟余量。
SINGLE_CHUNK_TIMEOUT = 600

# 实体/关系描述摘要的硬超时（秒）。摘要阶段在 gather 之后、原 wait_for 范围外，
# 若某个 summary 的 LLM 调用卡死会让整个合并阶段挂起（曾卡死 5 小时）。超时用原始描述兜底。
SUMMARY_TIMEOUT = 120
...
async def extract_entities(
    ...
    # 记录被跳过的 chunk：[(chunk_key, reason, content_preview)]
    skipped_chunks: list[tuple[str, str, str]] = []

    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        nonlocal already_processed, already_entities, already_relations, already_relations_low, already_relations_high, skipped_chunks
        chunk_key = chunk_key_dp[0]
        chunk_dp = chunk_key_dp[1]
        content = chunk_dp["content"]
        hint_prompt = entity_extract_prompt.format(**context_base, input_text=content)
        logger.debug(f"[chunk] 开始抽取 {chunk_key} | content[:80]={content[:80]!r}")
        logger.debug(f"[llm] hint_prompt[:200]={hint_prompt[:200]!r}")

        try:
            final_result = await use_llm_func(hint_prompt)
            if final_result is None:
                logger.warning(f"[chunk] {chunk_key} 主体抽取返回 None，跳过")
                return None,None,None,None
            logger.debug(f"[llm] final_result[:200]={final_result[:200]!r}")

            history = pack_user_ass_to_openai_messages(hint_prompt, final_result)
            for now_glean_index in range(entity_extract_max_gleaning):
                logger.debug(f"[glean {now_glean_index}] {chunk_key} 调 continue_extraction | prompt[:120]={continue_prompt[:120]!r}")
                glean_result = await use_llm_func(continue_prompt, history_messages=history)
                if glean_result is None:
                    logger.debug(f"[glean {now_glean_index}] {chunk_key} continue 返回 None，结束 glean")
                    break
                logger.debug(f"[glean {now_glean_index}] {chunk_key} continue 返回[:120]={glean_result[:120]!r}")
                ...
                logger.debug(f"[glean {now_glean_index}] {chunk_key} 调 if_loop_extraction")
                if_loop_result: str = await use_llm_func(
                    if_loop_prompt, history_messages=history
                )
                if_loop_result = if_loop_result.strip().strip('"').strip("'").lower()
                logger.debug(f"[glean {now_glean_index}] {chunk_key} if_loop={if_loop_result!r} -> {'继续' if if_loop_result=='yes' else '停止'}")
            ...
            logger.debug(f"[parse] {chunk_key} 拆出 {len(records)} 条 record")
            ...
            sys.stdout.flush()
            logger.debug(f"[chunk] {chunk_key} 完成: {len(maybe_nodes)} entities, {len(maybe_edges)} relations (low={len(maybe_edges_low)}, high={len(maybe_edges_high)})")
            return dict(maybe_nodes), dict(maybe_edges), dict(maybe_edges_low), dict(maybe_edges_high)
        except Exception as _e:
            # 判定跳过原因（所有可跳过的错误都刷屏打印 chunk 全文 + LLM 输入 prompt 全文 + 输入 token 数）
            reason = None
            if _is_unrecoverable_content_error(_e):
                reason = "内容审核"
            elif _is_transient_exhausted_error(_e):
                reason = f"瞬时错误({type(_e).__name__})"
            elif _is_llm_api_error(_e):
                reason = f"LLM错误({type(_e).__name__})"
            else:
                # 真 bug（KeyError/AttributeError 等）：不跳过，直接 raise 暴露问题
                raise

            # ---- 刷屏打印：chunk 全文 + LLM 输入 prompt 全文 + 输入 token 数 ----
            # 重算实体抽取的 hint_prompt（与正常处理时构造方式一致），数输入 token。
            # LLM 输出在错误/超时时不一定有，统一标注不打印（避免误导）。
            try:
                hint_prompt = entity_extract_prompt.format(**context_base, input_text=content)
                n_in_tokens = len(encode_string_by_tiktoken(
                    hint_prompt, model_name=global_config.get("tiktoken_model_name", "gpt-4o-mini")))
            except Exception:
                hint_prompt, n_in_tokens = "<hint_prompt 重算失败>", -1
            logger.warning(
                f"[chunk] {chunk_key} {reason}，跳过该 chunk（原文仍进 chunks_vdb）：{_readable_err_short(_e)}\n"
                f"  输入 token 数: {n_in_tokens}\n"
                f"  ---- LLM 输入 prompt 全文 ----\n{hint_prompt}\n"
                f"  ---- chunk 原文全文 ----\n{content}"
            )
            skipped_chunks.append((chunk_key, f"{reason}(in_tok={n_in_tokens})", content[:120].replace("\n", " ")))
            return None, None, None, None
    ...
    logger.info(f"[extract] 开始并发抽取 {len(ordered_chunks)} 个 chunk（max_async={global_config.get('llm_model_max_async')}，单 chunk 超时 {SINGLE_CHUNK_TIMEOUT}s）")

    async def _process_with_timeout(c):
        """给单个 chunk 加硬超时，根治 LLM 超时返回畸形结果导致的解析死循环/卡死。
        超时抛 asyncio.TimeoutError，由 _process_single_content 的 except 当作可跳过错误处理。"""
        try:
            return await asyncio.wait_for(_process_single_content(c), timeout=SINGLE_CHUNK_TIMEOUT)
        except asyncio.TimeoutError:
            chunk_key = c[0]
            content = c[1].get("content", "") if isinstance(c[1], dict) else ""
            # 超时时 LLM 还没返回，无法拿到「输出」和「输出 token」；但输入(prompt)+输入 token 可重算。
            # 重算实体抽取的 hint_prompt（与 _process_single_content 构造方式一致）
            try:
                hint_prompt = entity_extract_prompt.format(**context_base, input_text=content)
                n_in_tokens = len(encode_string_by_tiktoken(hint_prompt, model_name=global_config.get("tiktoken_model_name", "gpt-4o-mini")))
            except Exception:
                hint_prompt, n_in_tokens = "<hint_prompt 重算失败>", -1
            logger.warning(
                f"[chunk] {chunk_key} 处理超时(>{SINGLE_CHUNK_TIMEOUT}s，疑似死循环/卡死)，跳过该 chunk（原文仍进 chunks_vdb）\n"
                f"  输入 token 数: {n_in_tokens}\n"
                f"  输出 token 数: <超时未产出>\n"
                f"  LLM 输入 prompt 全文:\n{hint_prompt}\n"
                f"  LLM 输出: <超时未产出，无法打印>\n"
            ...
            skipped_chunks.append((chunk_key, f"处理超时(>{SINGLE_CHUNK_TIMEOUT}s, in_tok={n_in_tokens})", content[:120].replace("\n", " ")))
            return None, None, None, None

    results = await asyncio.gather(
        *[_process_single_content(c) for c in ordered_chunks ]
    )
    logger.info(f"[extract] 并发抽取完成，开始合并实体/关系并写入超图")

    # ---- 统计被跳过的 chunk（按原因分类 + 打印清单）----
    total = len(ordered_chunks)
    n_skipped = len(skipped_chunks)
    if n_skipped > 0:
        # 按原因分类计数
        from collections import Counter as _Counter
        reason_counts = _Counter(r for _, r, _ in skipped_chunks)
        logger.warning(
            f"[skip-stats] 共跳过 {n_skipped}/{total} 个 chunk（{n_skipped/total*100:.2f}%），"
            f"按原因：{dict(reason_counts)}"
        )
        # 打印每个被跳过的 chunk 清单
        skip_lines = ["[skip-list] 被跳过的 chunk 清单："]
        for i, (ck, reason, preview) in enumerate(skipped_chunks, 1):
            skip_lines.append(f"  {i:3d}. {ck} | {reason} | 内容预览: {preview!r}")
        logger.warning("\n".join(skip_lines))
    else:
        logger.info(f"[skip-stats] 无 chunk 被跳过（{total}/{total} 全部成功）")
