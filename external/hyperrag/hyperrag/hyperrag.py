...
class HyperRAG:
    ...
    # 单次 LLM 调用的硬超时（秒），只在「真正执行」（拿到并发名额后）阶段计时，排队不计。
    # None=不限。防止单次 LLM 卡死（如服务端不响应）拖垮整个 chunk 处理。
    llm_model_call_timeout: int = 180
    ...
    def __post_init__(self):
        ...
        set_logger(log_file, verbose=self.verbose)
        ...
        self.llm_model_func = limit_async_func_call(
            self.llm_model_max_async, timeout=self.llm_model_call_timeout
        )(
            partial(
                self.llm_model_func,
    ...
    async def ainsert(self, string_or_strings):
        try:
            ...
            logger.debug(f"[vdb] chunks 向量库 upsert 完成")
            # ----------------------------------------------------------------------------
            ...
            logger.debug(f"[hg] 知识超图更新完成，写入 full_docs/text_chunks")
            await self.full_docs.upsert(new_docs)
            await self.text_chunks.upsert(inserting_chunks)
            logger.info(f"[insert] 全部完成，开始落盘（index_done）")
        finally:
