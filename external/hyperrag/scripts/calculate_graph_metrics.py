#!/usr/bin/env python3
"""计算 Hyper-RAG 超图知识图谱的 Indexing 指标。

从 caches/ 读取 hypergraph_chunk_entity_relation.hgdb（超图 pickle），
将超图转换为普通无向图（超边内实体两两连边），然后计算 20 个图结构指标。
指标定义与 GraphRAG-Benchmark/Evaluation/indexing_eval.py 完全一致。

用法:
    cd external/hyperrag
...
def _hgdb_to_igraph(hgdb_path: str) -> ig.Graph:
    """将 Hypergraph-DB (.hgdb) 转换为 igraph 普通无向图。

    .hgdb 的 pickle 结构:
      - v_data: dict[entity_name] → {entity_type, description, ...}
      - v_inci: dict[entity_name] → set of (hyperedge entities tuple, ...)
      - e_data: dict[hyperedge_tuple] → {description, weight, ...}

    转换方式: 每条超边内的实体两两连边（clique expansion）。
    """
    with open(hgdb_path, "rb") as f:
        data = pickle.load(f)

    entities = list(data["v_data"].keys())
    vertex_map = {name: i for i, name in enumerate(entities)}
    num_hyper_v = len(entities)
    num_hyper_e = len(data["e_data"])

    edges: set[tuple[int, int]] = set()
    for he in data["e_data"].keys():
        members = [vertex_map[e] for e in he if e in vertex_map]
        for i in range(len(members)):
            for j in range(i + 1, len(members)):
                a, b = members[i], members[j]
                edges.add((min(a, b), max(a, b)))

    g = ig.Graph(n=num_hyper_v, edges=list(edges), directed=False)
    g.vs["name"] = entities
    g["hyper_vcount"] = num_hyper_v
    g["hyper_ecount"] = num_hyper_e
    return g


def load_graph(data_name: str, dataset_root: str) -> ig.Graph:
    path = os.path.join(dataset_root, data_name, "hypergraph_chunk_entity_relation.hgdb")
    if not os.path.isfile(path):
        raise FileNotFoundError(
            f"超图文件不存在: {path}\n请先运行 reproduce/Step_1_build_index.py 构建索引"
        )
    return _hgdb_to_igraph(path)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Hyper-RAG 图指标计算（hgdb → igraph，对标 judge indexing_eval）"
    )
    ...
    hyper_v = g["hyper_vcount"]
    hyper_e = g["hyper_ecount"]
    if args.verbose:
        print(f"[{args.data_name}] hypergraph: {hyper_v} entities, {hyper_e} hyperedges")
        print(f"[{args.data_name}] clique graph: {g.vcount()} nodes, {g.ecount()} edges")

    metrics = analyze_graph(g)
    metrics["dataset"] = args.data_name
    metrics["hypergraph_entities"] = float(hyper_v)
    metrics["hypergraph_hyperedges"] = float(hyper_e)
