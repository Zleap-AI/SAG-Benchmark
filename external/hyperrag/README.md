# Hyper-RAG — sag-benchmark External Method

Uses iMoonLab hyperrag-benchmark for hypergraph-based QA.

## Setup

Run all commands from this project directory (`external/hyperrag/` of the
repository root):
...
| `LLM_FUNC_MAX_ASYNC` | `16` | Max concurrent LLM calls (falls back to `MAX_ASYNC`; kernel default 16) |

### LLM sampling & behavior (built-in)

LLM 采样/行为参数不再从 `.env` 读取，内建在 `hyperrag_config.py` 的
...
Supports modes: `naive`, `hyper`, `hyper-lite`.
...
uv run --frozen --env-file ../../.env python reproduce/Step_3_response.py --data_name test_hotpotqa --mode hyper
```

## Output Locations

索引（图/向量库）在 `caches/<ds>/`，QA 响应在 `outputs/<ds>/response/<run_id>/`——同一张图可以跑多次 QA，每次一个 `run_id` 目录，`manifest.json` 记录本次响应用了哪个索引、mode 和什么检索参数。

| Artifact | Path |
|----------|------|
| Index | `caches/<ds>/`（graphml/vdb/kv_store_*） |
| Response (naive) | `outputs/<ds>/response/<run_id>/naive_<ds>_result.json` |
| Response (hyper) | `outputs/<ds>/response/<run_id>/hyper_<ds>_result.json` |
| Response (hyper-lite) | `outputs/<ds>/response/<run_id>/hyper-lite_<ds>_result.json` |
...
uv run --frozen --env-file ../../.env python reproduce/Step_3_response.py \
    --data_name test_hotpotqa --mode hyper --run-id exp_v2 --no_resume

# 或不命名，自动新建一个时间戳 run（YYYYMMDD_HHMMSS）
uv run --frozen --env-file ../../.env python reproduce/Step_3_response.py \
    --data_name test_hotpotqa --mode hyper --no_resume

# 中断后继续某个 run（只补跑该 run 里失败/缺漏的题）
uv run --frozen --env-file ../../.env python reproduce/Step_3_response.py \
    --data_name test_hotpotqa --mode hyper --resume-run-id exp_v2
```
...
After Step 3, run both commands from the repository root. The conversion mode
must match the Step 3 `--mode` value.
...
`evaluation/hyperrag/<ds>/<source_run_id>/llmjudge/<judge-model>/<judge-run-id>/`.
