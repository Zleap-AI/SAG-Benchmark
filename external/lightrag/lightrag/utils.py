...
def write_json(json_obj, file_name):
    """Atomically write JSON so a crash mid-write never leaves a partial file."""
    dirname = os.path.dirname(file_name) or "."
    fd, tmp = tempfile.mkstemp(suffix=".json", dir=dirname, text=True)
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(json_obj, f, indent=2, ensure_ascii=False)
        os.replace(tmp, file_name)  # POSIX atomic rename
