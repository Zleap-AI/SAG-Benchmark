# LightRAG — sag-benchmark External Method

Uses HKUDS LightRAG for graph-based RAG with knowledge graphs and vector retrieval.

## Setup

Run all commands from this project directory (`external/lightrag/` of the
repository root):
...
| `EMBED_TOKENIZER_URL` | unset | Optional embedding tokenizer endpoint (defaults to the embedding base URL with `/v1` stripped) |
| `CHUNK_SIZE` | `1200` | Chunk token size for indexing |
| `CHUNK_OVERLAP` | `100` | Chunk overlap token size for indexing |
| `EMBEDDING_FUNC_MAX_ASYNC` | `16` | Max concurrent embedding calls |
| `LLM_FUNC_MAX_ASYNC` | `16` | Max concurrent LLM calls (falls back to `MAX_ASYNC`) |
| `MAX_PARALLEL_INSERT` | `20` | Docs inserted per parallel batch — lower it to ease server pressure |

### LLM sampling & behavior (built-in)

LLM 采样/行为参数不再从 `.env` 读取，内建在 `lightrag_config.py` 的
...
`evaluation/lightrag/<ds>/<source_run_id>/llmjudge/<judge-model>/<judge-run-id>/`.
