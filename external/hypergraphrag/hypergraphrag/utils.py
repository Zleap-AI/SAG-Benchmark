...
def locate_json_string_body_from_string(content: str) -> Union[str, None]:
    ...
    Two-pass strategy:
      1. Try to extract and parse JSON as-is (model uses proper double quotes).
         This path preserves apostrophes inside string values like "don't".
      2. If that fails, fall back to replacing single quotes with double quotes
         (older prompts may output single-quoted JSON).
    """
    try:
        maybe_json_str = re.search(r"{.*}", content, re.DOTALL)
        if maybe_json_str is None:
            return None
        maybe_json_str = maybe_json_str.group(0)
        maybe_json_str = maybe_json_str.replace("\\n", "")
        maybe_json_str = maybe_json_str.replace("\n", "")

        # Pass 1: try as-is (proper double-quoted JSON)
        try:
        ...
        # Pass 2: fall back to single-quote -> double-quote replacement
        fallback = maybe_json_str.replace("'", '"')
        try:
            json.loads(fallback)
            return fallback
        except json.JSONDecodeError:
...
def limit_async_func_call(max_size: int, waitting_time: float = 0.0001):
    ...
    def final_decro(func):
        ...
        async def wait_func(*args, **kwargs):
            ...
            __current_size += 1
            # local patch: release the slot even when func raises, otherwise a
            # single exception (e.g. APITimeoutError) permanently leaks one slot
            # and the effective concurrency degrades to 1 over time.
            try:
