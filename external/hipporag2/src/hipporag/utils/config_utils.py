...
class BaseConfig:
    ...
    max_new_tokens: Union[None, int] = field(
        default=2048,
        metadata={"help": "Max new tokens to generate in each inference (used for OpenIE / indexing calls)."}
    )
    qa_max_new_tokens: Union[None, int] = field(
        default=4096,
        metadata={"help": "Max new tokens specifically for QA inference. Should be larger than max_new_tokens "
                          "to prevent response truncation before 'Answer:' is generated on complex multi-hop questions. "
                          "If None, falls back to max_new_tokens."}
    )
    qa_temperature: float = field(
        default=0.0,
        metadata={"help": "Temperature specifically for QA inference. Defaults to 0 for deterministic, reproducible "
                          "answers; other stages (e.g. OpenIE / indexing) still use the global `temperature` (default 0.7)."}
    )
    ...
    embedding_base_url: str = field(
        default=None,
        metadata={"help": "Base URL for the embedding model API (OpenAI-compatible endpoint)."}
    )
    ...
    dataset: Optional[Literal['hotpotqa', 'hotpotqa_train', 'musique', '2wikimultihopqa', 'narrativeqa']] = field(
        default=None,
