...
class DSPyFilter:
    ...
    def llm_call(self, question, fact_before_filter):
        ...
        if len(response) > 1:
            return response[0], response[1]  # (text, metadata)
        return response, {}
    ...
    def rerank(self,
        ...
        rerank_metadata = {}  # collect token usage
        try:
            # prediction = self.program(question=query, fact_before_filter=json.dumps(fact_before_filter))
            response, rerank_metadata = self.llm_call(query, json.dumps(fact_before_filter))
        ...
        return sorted_candidate_indices[:len_after_rerank], sorted_candidate_items[:len_after_rerank], {
            'confidence': None,
            'prompt_tokens': rerank_metadata.get('prompt_tokens', 0),
            'completion_tokens': rerank_metadata.get('completion_tokens', 0),
        }