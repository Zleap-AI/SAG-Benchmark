...
class HippoRAG:
    ...
    def __init__(self, global_config=None, save_dir=None, llm_model_name=None, embedding_model_name=None, llm_base_url=None, embedding_base_url=None):
        """
        ...
        Attributes:
            ...
            openie (OpenIE): The online Open Information Extraction module.
        ...
        # SAG-Benchmark external 集成仅支持在线 OpenIE（OpenAI 兼容端点）
    ...
    def retrieve(self,
        ...
        """
        with cost_tracker.phase("retrieval"):
            ...
            for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
                ...
                retrieval_prompt_tokens += rerank_log.get('prompt_tokens', 0)
                retrieval_completion_tokens += rerank_log.get('completion_tokens', 0)
            ...
            logger.info(f"Retrieval LLM token usage: prompt={retrieval_prompt_tokens}, completion={retrieval_completion_tokens}, total={retrieval_prompt_tokens + retrieval_completion_tokens}")
    ...
    def qa(self, queries: List[QuerySolution]) -> Tuple[List[QuerySolution], List[str], List[Dict]]:
        ...
        with cost_tracker.phase("qa"):
            ...
            # Use a dedicated QA max_tokens that's larger than the global default to avoid
            # response truncation before "Answer:" is generated (especially for multi-hop questions
            # that require long reasoning chains).
            # NOTE: uses 'max_completion_tokens' kwarg so it overrides generate_params in CacheOpenAI.infer()
            # NOTE: QA 用专属 qa_temperature（默认 0，确定性、可复现），经 kwargs 覆盖全局 temperature(0.7)，
            #       并参与缓存 key，避免与其它阶段的缓存命中混淆。
            qa_max_tokens = getattr(self.global_config, 'qa_max_new_tokens', None) or \
                            max(getattr(self.global_config, 'max_new_tokens', 2048), 2048)
            qa_temperature = getattr(self.global_config, 'qa_temperature', 0.0)

            all_qa_results = []
            for qa_messages in tqdm(all_qa_messages, desc="QA Reading"):
                try:
                    result = self.llm_model.infer(qa_messages, max_completion_tokens=qa_max_tokens, temperature=qa_temperature)
                    all_qa_results.append(result)
                except Exception as e:
                    logger.warning(f"QA infer 失败，跳过该条（将以空字符串作为答案）: {e}")
                    # 构造一个空响应占位，保持列表长度与 queries 一致
                    all_qa_results.append(("", {"prompt_tokens": 0, "completion_tokens": 0, "finish_reason": "error"}, False))

            all_response_message, all_metadata, all_cache_hit = zip(*all_qa_results)
            all_response_message, all_metadata = list(all_response_message), list(all_metadata)

            qa_prompt_tokens = sum(m.get('prompt_tokens', 0) for m in all_metadata)
            qa_completion_tokens = sum(m.get('completion_tokens', 0) for m in all_metadata)
            logger.info(f"QA LLM token usage: prompt={qa_prompt_tokens}, completion={qa_completion_tokens}, total={qa_prompt_tokens + qa_completion_tokens}")
            ...
            for query_solution_idx, query_solution in tqdm(enumerate(queries), desc="Extraction Answers from LLM Response"):
                ...
                try:
                    if 'Answer:' in response_content:
                        # Use [-1] to get the LAST occurrence of "Answer:" (model may write "Answer:" in its reasoning too)
                        pred_ans = response_content.split('Answer:')[-1].strip()
                        # Take only the first line of the answer (ignore any subsequent lines)
                        pred_ans = pred_ans.split('\n')[0].strip()
                    else:
                        # "Answer:" is missing — model response likely truncated due to max_tokens limit
                        # (finish_reason == "length"). Try common answer-conclusion patterns as fallback.
                        finish_reason = metadata.get('finish_reason', 'unknown') if isinstance(metadata, dict) else 'unknown'
                        if finish_reason == 'length':
                            logger.warning(
                                f"QA response #{query_solution_idx} was truncated (finish_reason=length, "
                                f"completion_tokens={metadata.get('completion_tokens', '?')}). "
                                f"'Answer:' keyword not found. Trying fallback extraction."
                            )
                        answer_pattern = re.search(
                            r'(?:so the answer is|therefore[,\s]+the answer is|the answer is|answer is)[:\s]+([^\n.?!]+)',
                            response_content,
                            re.IGNORECASE
                        )
                        if answer_pattern:
                            pred_ans = answer_pattern.group(1).strip().rstrip('.,')
                            logger.info(f"QA response #{query_solution_idx}: extracted answer via fallback pattern: '{pred_ans}'")
                        else:
                            parse_fail_count += 1
                            logger.warning(
                                f"QA response #{query_solution_idx}: 'Answer:' not found and fallback pattern also failed. "
                                f"Using full response as answer. Hint: increase max_new_tokens or simplify the QA prompt."
                            )
            ...
            if parse_fail_count > 0:
                logger.warning(
                    f"{parse_fail_count}/{len(queries)} QA responses could not be parsed. "
                    f"To fix this: increase max_new_tokens (currently {getattr(self.global_config, 'max_new_tokens', 'unknown')}) "
                    f"or modify the QA prompt to request shorter reasoning."
                )
    ...
    def prepare_retrieval_objects(self):
        ...
        n_entity = len(self.entity_node_keys)
        n_passage = len(self.passage_node_keys)
        n_graph = self.graph.vcount()
        if n_entity + n_passage != n_graph:
            graph_names = set(self.graph.vs["name"]) if "name" in self.graph.vs.attribute_names() else set()
            store_names = set(self.entity_node_keys) | set(self.passage_node_keys)
            only_in_graph = sorted(graph_names - store_names)[:5]
            only_in_store = sorted(store_names - graph_names)[:5]
            raise AssertionError(
                f"索引产物不一致: entity({n_entity}) + passage({n_passage}) = {n_entity + n_passage} "
                f"!= graph.vcount({n_graph})。"
                f" 图中存在但 embedding store 缺失的节点(示例): {only_in_graph}"
                f"  embedding store 存在但图中缺失(示例): {only_in_store}"
                f"  请重新运行 Step_1_build_index.py --data_name {self.global_config.dataset} 完整重建索引"
                f"（Step_3 传 --force 亦可强制重建），不要重复调用 index() 导致 graph/embedding 不同步。"
            )
        ...
        # Rebuild ent_node_to_num_chunk from graph edges (normally done in index(),
        # but needed here for search-only / qa_only paths that skip index).
        if not hasattr(self, 'ent_node_to_num_chunk') or not self.ent_node_to_num_chunk:
            passage_set = set(self.passage_node_idxs)
            self.ent_node_to_num_chunk = {}
            for entity_key in self.entity_node_keys:
                entity_idx = self.node_name_to_vertex_idx.get(entity_key)
                if entity_idx is not None:
                    neighbors = self.graph.neighbors(entity_idx)
                    self.ent_node_to_num_chunk[entity_key] = sum(1 for n in neighbors if n in passage_set)
    ...
    def rerank_facts(self, query: str, query_fact_scores: np.ndarray) -> Tuple[List[int], List[Tuple], dict]:
        ...
        rerank_log = {
            ...
            'prompt_tokens': reranker_dict.get('prompt_tokens', 0),
            'completion_tokens': reranker_dict.get('completion_tokens', 0),
        }
