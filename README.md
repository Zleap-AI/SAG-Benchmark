...
将非结构化文本转化为独立的**事项单元**（Event），每个事项包含完整的上下文信息，作为检索的基本单位。与传统的文档分块（Chunk）相比，事项具有更强的语义完整性和可索引性。

#### 2. 超图结构 vs 传统三元组知识图谱

**传统知识图谱的局限：**
- **三元组表示**：`(实体1, 关系, 实体2)` 只能表达两个实体之间的二元关系
- **复杂关系难以建模**：多实体参与的事件（如"A、B、C 三人共同完成项目 X"）需要拆分为多个三元组，破坏了事件的整体性
- **维护成本高**：需要预定义关系类型，难以适应动态变化的知识

**SAG 的超图优势：**
- **超边（Hyperedge）表示**：一个事项可以关联任意数量的实体，自然表达多元关系
- **事项为中心**：`Event(实体1, 实体2, ..., 实体N)` 保持事件的完整语义
- **动态计算关联**：无需预定义关系类型，通过向量相似度和 LLM 动态发现实体间的关联
- **降低维护成本**：不需要维护复杂的关系本体（Ontology），适应知识的持续演化

**示例对比：**

| 对比维度 | 三元组知识图谱 | SAG 超图 |
|---------|--------------|---------|
| **事件描述** | "张三、李四、王五在 2023 年共同创立了 AI 公司" | 同左 |
| **如何建模** | 拆分为 4 个三元组：<br>`(张三, 创立, AI公司)`<br>`(李四, 创立, AI公司)`<br>`(王五, 创立, AI公司)`<br>`(AI公司, 成立时间, 2023)` | 一个超边表示完整事件：<br>`Event_1(张三, 李四, 王五, AI公司, 2023)` |
| **查询"张三和李四有什么关系？"** | 需要分别查询两人与 AI公司 的关系，再合并结果推理出共同创始人关系 | 直接从 Event_1 中发现两人共同参与创立事件 |
| **添加新参与者** | 需要创建新三元组 `(新人, 创立, AI公司)` | 直接在 Event_1 中添加新实体 |

#### 3. 动态实体关联

实体与事项的关联关系存储在 MySQL 中（`event_entity` 关联表），但**关联的发现和计算是动态的**：
- 在知识提取阶段，通过 **LLM** 从文本中识别实体并建立与事项的关联
- 在检索阶段，通过 **Embedding 向量相似度** 动态计算实体之间的语义关联强度
- 无需预先定义关系类型（如"创立"、"参与"、"影响"等），系统自动发现隐含的实体关联
...
支持多维度实体标注（人物、地点、时间、组织、概念等），每个事项可以关联多个不同维度的实体，形成丰富的语义网络。

#### 5. 智能召回机制

结合 **向量检索**（快速粗排）、**多跳图扩展**（关联发现）和 **LLM 精排**（语义理解），实现高召回率和高精度的平衡。
...
│   │   ├── source/         # upload 生成的 source_info.json
