...
def create_storage_facade(settings: Settings) -> StorageFacade:
    ...
    return StorageFacade(
        ...
        vector=vector,
        search=search,
        event_universe=database,
        # BM25 remains Elasticsearch-backed even when vectors are stored in
        # OceanBase. The provider is lazy, so other strategies open no ES
        # connection for this optional capability.
        chunk_text=ElasticsearchChunkTextSearchStore(),
    )
