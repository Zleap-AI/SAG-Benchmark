...
class EntityVectorRepository(BaseRepository):
    ...
    def __init__(self, es_client: AsyncElasticsearch, embedding_dim: int | None = None):
        ...
        """
        super().__init__(es_client, embedding_dim=embedding_dim)
    ...
    async def search_by_name(
        self, name: str, source_config_id: str | None = None, size: int = 10
    ) -> list[dict[str, Any]]:
        """
        按名称搜索实体
    ...
    async def search_by_query_bm25(
        ...
        if allowed_entity_ids:
            filter_clauses.append({"terms": {"entity_id": allowed_entity_ids}})
    ...
    async def _get_entity_type_info(
        self, source_config_ids: list[str] | None = None
    ...
    async def search_similar(
        ...
        if allowed_entity_ids:
            filters.append(Q("terms", entity_id=allowed_entity_ids))
    ...
    async def search_by_names_exact(
        ...
        query = {
            "bool": {"should": should_clauses, "minimum_should_match": 1, "filter": filter_clauses}
        }
    ...
    async def get_by_source(
        self, source_config_id: str, entity_type: str | None = None, size: int = 100
    ) -> list[dict[str, Any]]:
        """
        获取信息源的所有实体
    ...
    async def get_entities_by_ids(self, entity_ids: list[str]) -> list[dict[str, Any]]:
        ...
        for i in range(0, len(entity_ids), BATCH_SIZE):
            ...
            # 构建ES查询
            query = {"terms": {"entity_id": batch_ids}}
