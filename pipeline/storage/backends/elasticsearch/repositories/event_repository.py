...
from pipeline.storage.backends.elasticsearch.index_naming import BASE_INDEX_EVENT_VECTORS
...
class EventVectorRepository(BaseRepository):
    ...
    async def search_by_text(
        self, query: str, source_config_id: str | None = None, size: int = 10
    ) -> list[dict[str, Any]]:
        """
        全文检索
        ...
        try:
            ...
            if hits:
                ...
                if isinstance(hits[0], dict):
                    ...
                    event_data = hits[0].get("source", hits[0].get("_source", {}))
                else:
                    # Non-standard format: hit might be an object with attributes
                    event_data = getattr(hits[0], "source", getattr(hits[0], "_source", {}))
    ...
    async def get_events_by_ids(
        ...
        Args:
            event_ids: 事件ID列表
            source_includes: 可选的 _source 白名单字段；不传时保持旧行为
        ...
        source_filter = (
            {"includes": source_includes} if source_includes else {"excludes": ["title_vector"]}
        )

        # 默认保持旧行为；调用方可传 source_includes 减少数据传输。
        query_body = {
    ...
    async def search_bm25_by_text(
        ...
        """
        在指定 event_id 候选集合内做 BM25 文本检索。

        Args:
            query: 查询文本
            event_ids: 候选事项 ID 列表
            k: 返回数量
            source_config_ids: 信息源 ID 列表
            fields: BM25 查询字段，例如 ["title^2", "content"]
        ...
        routing = (
            source_config_ids[0] if source_config_ids and len(source_config_ids) == 1 else None
        )
    ...
    async def _vector_search(
        ...
        if source_config_ids:
            ...
            if len(source_config_ids) == 1:
                must_filters.append(Q("term", source_config_id=source_config_ids[0]).to_dict())
            else:
                must_filters.append(Q("terms", source_config_id=source_config_ids).to_dict())
        if category:
    ...
    async def search_by_entities(
        self, entity_ids: list[str], source_config_id: str | None = None, size: int = 100
    ) -> list[dict[str, Any]]:
        """
        按关联实体搜索事件
