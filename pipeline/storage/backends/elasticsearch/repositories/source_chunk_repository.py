...
from pipeline.storage.backends.elasticsearch.index_naming import BASE_INDEX_SOURCE_CHUNKS
...
class SourceChunkRepository(BaseRepository):
    ...
    async def index_chunk(
        ...
        heading: str | None,
        content: str,
        heading_vector: list[float] | None,
    ...
    async def search_by_text(
        ...
        # 仅对 content 正文做 BM25 检索（不再对 heading 打分/加权）
        # minimum_should_match="0%"：等价于默认 operator=or（任一词命中即进结果），
        # 不做词命中比例约束。若长查询噪声明显，可调高（如 "30%"）加一层防御，
        # 但实测过高（≥50%）会误伤 gold 文档，需按数据集权衡。
        s = s.query(
            "multi_match",
            query=query,
        ...
        routing = source_config_id if source_config_id else None
        # 用 return_full_response=True 拿到 BM25 _score（默认 False 只返回 _source 扁平列表）
        response = await self.es_client.search(
            index=self.INDEX_NAME,
            query=search_dict.get("query", {}),
            size=search_dict.get("size", 10),
        ...
        # return_full_response=True 结构: {total, max_score, hits:[{id, score, source, index}]}
        # 保留 ES 原始 BM25 分（_score），其余字段取 source
        results: list[dict[str, Any]] = []
        for hit in response.get("hits", []):
            item = dict(hit.get("source") or {})
            item["_score"] = float(hit.get("score") or 0.0)
            results.append(item)
        return results

    async def get_chunks_by_ids(
        self, chunk_ids: list[str], include_vectors: bool = True
    ) -> list[dict[str, Any]]:
        """
        ...
        # 构建ES查询
        query_body = {"query": {"terms": {"chunk_id": chunk_ids}}, "size": len(chunk_ids)}

        # 如果不需要向量，排除向量字段
        if not include_vectors:
            query_body["_source"] = {"excludes": ["heading_vector", "content_vector"]}
