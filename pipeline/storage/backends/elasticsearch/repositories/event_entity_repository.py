...
from pipeline.storage.backends.elasticsearch.index_naming import BASE_INDEX_EVENT_ENTITY_VECTORS
...
class EventEntityRepository(BaseRepository):
    ...
    async def get_by_event(
        self, event_id: str, source_config_id: str | None = None
    ) -> list[dict[str, Any]]:
        """
        获取事件的所有实体关联
    ...
    async def get_by_entity(
        self, entity_id: str, source_config_id: str | None = None
    ) -> list[dict[str, Any]]:
        """
        获取实体的所有事件关联
    ...
    async def get_event_ids_by_entity_ids(
        ...
        """
        根据实体 ID 列表批量获取关联事件 ID，并按 event_id 去重。

        使用 event_entity_vectors.source_config_id 过滤信息源，避免再 join
        SourceEvent；使用 collapse on event_id 实现 ES 侧去重。
        """
        if not entity_ids:
            return []
    ...
    async def get_associations_by_ids(self, association_ids: list[str]) -> list[dict[str, Any]]:
        """
        根据关联ID列表批量获取
