...
当前 Step1 使用 query BM25 召回实体，Step2 只生成 query embedding 供后续复用。
...
class MultiSearcherES:
    ...
    def _resolve_search_mode(self, config: Optional[Any]) -> str:
        ...
        fast:   快速模式，使用 BM25 实体召回 + 小规模扩展
        precise: 精准模式，使用 BM25 实体召回 + LLM 过滤
    ...
    async def step1_retrieve_entities_bm25(
        ...
        *,
        ...
        这里只做 top-k 截断，不使用 key_similarity_threshold。BM25 原始分数只用于
        同一次 query 内排序。
        """
    ...
    async def step3_retrieve_events(
        ...
        *,
        ...
        Args:
            ...
            entity_event_top_k: 通道1 entity→event 最大数量；不传时复用 multi_top_k
