...
class MultiConfig(pipelineBaseModel):
    ...
    mode: Literal["fast", "precise"] = Field(
        default="fast",
        description="multi_vector 模式：fast=spaCy+小规模扩展，precise=spaCy+LLM过滤"
    )
    spacy_model: str = Field(
        default="en_core_web_sm",
        description="[fast] spaCy 模型名称"
    )
    ...
    max_expand_events_per_hop: int = Field(
        default=2000,
        ge=1, le=10000,
        description="[multi] 扩展阶段每跳 entity->event 最多召回事项数量"
    )
    ...
    fast_entity_k: int = Field(
        default=5,
        ge=1, le=100,
        description="[fast] key->entity 后保留的实体数量"
    )
    fast_entity_event_candidate_k: int = Field(
        default=2000,
        ge=1, le=10000,
        description="[fast] entity->event 初始候选数量，随后按 query-content 相似度取 fast_entity_event_k"
    )
    ...
    fast_query_event_k: int = Field(
        default=20,
        ge=1, le=1000,
        description="[fast] query->event2 直接向量召回数量"
    )
    ...
    fast_channel_weight: float = Field(
        default=0.05,
        ge=0.0,
        description="[fast] expand 前第一跳事项排序的双通道命中奖励权重"
    )
