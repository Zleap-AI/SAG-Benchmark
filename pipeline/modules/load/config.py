...
class LoadBaseConfig(PipelineBaseModel):
    ...
    # === 通用配置 ===
    max_tokens: int = Field(default=1000, ge=100, le=100000, description="每个chunk的最大token数")

    # === 存储配置 ===
    auto_vector: bool = Field(default=True, description="是否自动索引到Elasticsearch")

    # === 提示词增强 ===
    background: str | None = Field(default=None, description="背景信息（补充元数据生成上下文）")

    # === 信息源 ===
    source_config_id: str | None = Field(default=None, description="信息源ID")
    ...
    # === 批量处理配置 ===
    enable_batch_indexing: bool = Field(default=True, description="是否启用批量索引优化")

    embedding_batch_size: int = Field(
        default=10, ge=1, le=100, description="向量生成批量大小（每批处理的文本数量）"
    )

    es_bulk_index_size: int = Field(
        default=50, ge=1, le=200, description="ES批量索引大小（每批索引的文档数量）"
    )
...
class DocumentLoadConfig(LoadBaseConfig):
    ...
    # === 数据来源 ===
    path: str | Path | None = Field(default=None, description="文件路径")

    # === 文档处理配置 ===
    min_content_length: int = Field(default=100, ge=10, description="最小内容长度（字符数）")
    merge_short_sections: bool = Field(default=False, description="是否启用短片段合并")
