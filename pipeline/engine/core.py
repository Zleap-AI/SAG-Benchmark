...
from pipeline.engine.config import ModelConfig, TaskConfig
...
class PipelineEngine:
    ...
    def __init__(
        ...
        stage_callback: Callable[[str, Any], Awaitable[None] | None] | None = None,
    ):
        ...
        self._source_config_id: str | None = source_config_id or (
            task_config.source_config_id if task_config else None
        )
    ...
    async def aclose(self) -> None:
        """Release resources owned by this engine's lazily created search runtime."""
    ...
    def _log(self, stage: TaskStage, level: LogLevel, message: str, extra: dict | None = None):
    ...
    async def extract_async(self, config: ExtractBaseConfig | None = None) -> None:
        ...
        async with self._stage(TaskStage.SEARCH, TaskStatus.SEARCHING) as elapsed:
            ...
            self.result.search_result = StageResult(
                ...
                stats={
                    ...
                    "rank_event_ids": raw.get("rank_event_ids", []),
                    "display_event_ids": raw.get("display_event_ids", []),
                    "rank_scores": raw.get("rank_scores", {}),
                    "score_top_event_ids": raw.get("score_top_event_ids", []),
                    "expand_hops": raw.get("expand_hops", []),
                    "recall_clue_stats": raw.get("recall_clue_stats", {}),
                    "chunk_headings": raw.get("chunk_headings", {}),
                    # 支持分阶段计时的策略会在此透传；其它策略为空。
                    "timing_steps": (raw.get("stats") or {}).get("timing_steps", {}),
                },
                duration=elapsed(),
            )
    ...
    def chunk_count(self) -> int:
        return (
            len(self._load_result.chunk_ids)
            if self._load_result and self._load_result.chunk_ids
            else 0
        )
