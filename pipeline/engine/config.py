...
class OutputConfig(BaseModel):
    ...
    export_path: Path | None = Field(default=None, description="导出文件路径")
...
class TaskConfig(BaseModel):
    ...
    task_description: str | None = Field(default=None, description="任务描述")

    # 运行时上下文 (全局配置)
    source_config_id: str | None = Field(default=None, description="信息源ID")
    source_name: str | None = Field(default=None, description="信息源名称")
    background: str | None = Field(default=None, description="全局背景信息（可被阶段覆盖）")

    # 各阶段配置 (基础配置)
    load: DocumentLoadConfig | None = Field(default=None, description="加载阶段配置")
    extract: ExtractBaseConfig | None = Field(default=None, description="提取阶段配置")
    search: SearchBaseConfig | None = Field(default=None, description="搜索阶段配置")
