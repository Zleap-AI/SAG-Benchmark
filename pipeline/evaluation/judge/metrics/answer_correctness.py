"""Answer Correctness — migrated from external/judge/Evaluation/metrics/answer_accuracy.py.
...
async def compute_answer_correctness(
    ...
    factuality_result = await _calculate_factuality(
        llm, question, answer_statements, gt_statements, beta,
...
async def _generate_statements(
    llm: Any, question: str, answer: str
) -> list[str]:
    prompt = prompts.STATEMENT_GENERATOR_PROMPT.format(
...
async def _calculate_factuality(
    ...
    try:
        ...
        empty = {"TP": [], "FP": [], "FN": []}
        return (0.0, empty) if return_intermediate else 0.0
